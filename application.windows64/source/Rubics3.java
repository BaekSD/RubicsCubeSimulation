...
public class Rubics3 extends PApplet {





float x = -0.44f+2*PI, y = -0.46f+2*PI, angle=0.0f;
int len = 500;
int size = 50;
...
public void mouseDragged()
{
  if (mouseButton == LEFT)
  {
    y += (mouseX-pmouseX)/(len/10.0f);
    x -= (mouseY-pmouseY)/(len/10.0f);
  }
  if (x < 0)
  {
    x += 2*PI;
...
class MatlabCube
{
  ...
  MatlabProxy proxy;
  double a, b, ccc[][] = new double[26][6];

  public void setting() throws MatlabConnectionException, MatlabInvocationException
  {
    proxy = factory.getProxy();
    proxy.eval("variables");
  }

  public void eval(String str) throws MatlabConnectionException, MatlabInvocationException
  {
    proxy.eval(str);
  }

  public void solving() throws MatlabConnectionException, MatlabInvocationException
  {
  ...
  public void returnCubeStatus() throws MatlabConnectionException, MatlabInvocationException
  {
...
class Cube
{
  ...
  public void drawCube(int s, float angle)
  {
    for (int i=0; i<cp.length; i++)
    {
...
class CubePart
{
  ...
  public void drawPart(int s, float angle)
  {
    for (int i=0; i<cpp.length; i++)
    {
...
class CubePartPart
{
  ...
  public void drawPartPart(int s, float angle)
  {
    if (s==1 && y==0)
    {
      rotateY(-angle);
    ...
    switch(side)
    {
      /*case 1:
       ...
       translate(0, -75, -z*50);*/
    case 1:
      translate(0, -size*1.5f, z*size);
      rotateX(PI/2.0f);
      setColor(p_color);
      rect((x-1.5f)*size, (y-1.5f)*size, size, size);
      noFill();
      rotateX(-PI/2.0f);
      translate(0, size*1.5f, -z*size);
      break;
    case 2:
      translate(0, 0, size*1.5f);
      setColor(p_color);
      rect((x-1.5f)*size, (y-1.5f)*size, size, size);
      noFill();
      translate(0, 0, -size*1.5f);
      break;
    case 3:
      translate(size*1.5f, 0, (z-2)*size);
      rotateY(-PI/2.0f);
      setColor(p_color);
      rect((x-1.5f)*size, (y-1.5f)*size, size, size);
      noFill();
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Rubics3" };
    if (passedArgs != null) {
